...
public class TresEnRaia {
    ...
    public static void jugar() {
        ...
        while (!finPartida(tablero, vacio)) { // mientras que la partida no haya finalizado o bien por que no quedan mas huecos o un jugador ya gano
            do {
                ...
                fila = pedirInteger("Inroduzca la fila"); //pide en que fila querra colocar su ficha 
                columna = pedirInteger("Inroduzca la columna");// pide en que columna querra colocar la ficha
                posValida = validarPosicion(tablero, fila, columna); // valida si la fila y columna introducida son correctas  
                if (posValida) {
                    if (posicionVacia(tablero, fila, columna, vacio)) {// valida si no hay alguna ficha ya puesta en esa posicion
                        correcto = true;
                    } else {
            ...
            } while (!correcto);// mientras que no se pueda poner la ficha por que los datos introducidos no son correctos, pedira contantemente los datos al usuario de nuevo
...
// diagonal principal
    ...
    public static void mostrarMatriz(char[][] matriz) {
    	...
    	for (int i = 0; i < matriz.length; i++) { // las i representan las filas
      	  System.out.print("|" +i+"| ");// muestra para referencia del jugador con que numero se representan las filas
